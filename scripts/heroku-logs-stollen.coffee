  ...
  new cron '10 * * * * *', () ->
    if !accountKey || !logAddr || !room
      robot.logger.error "env missing" + \
        " HUBOT_LOGENTRIES_STOLLEN_KEY: " + \
        process.env.HUBOT_LOGENTRIES_STOLLEN_KEY + \
        " HUBOT_LOGENTRIES_STOLLEN_KEY_ADDR: " + \
        process.env.HUBOT_LOGENTRIES_STOLLEN_KEY_ADDR + \
        " HUBOT_LOGENTRIES_STOLLEN_ROOM: " + \
        process.env.HUBOT_LOGENTRIES_STOLLEN_ROOM
      return
